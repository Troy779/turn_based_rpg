...
class Monster(Character):
    def __init__(self, name, health, drop_items):
        super().__init__(name, health, attack_power=0)  # Can increase if a specific attack is set
...
def main():
    ...
    enemies = [
        Monster("Fierce Goblin", 100, [("Goblin Dagger", 40, 25, "Common"),
                                        ("Goblin's Club", 20, 15, "Uncommon")]),
        Monster("Fire Dragon", 150, [("Dragon Scale", 50, 35, "Rare"),
                                      ("Fire Gem", 10, 0, "Epic"),
    ...
    for enemy in enemies:
        ...
        while player.is_alive() and enemy.is_alive():
            ...
            if action == '1':
                player.attack(enemy)
                if enemy.is_alive():
                    enemy.take_damage(player.attack_power)  # Monster attacks back
            elif action == '2':
                player.heal()
            else:
